 ...
 * @returns {Promise<{ok: boolean, error?: string, gateway?: unknown}>}
 */
export async function sendLeadSms({ name, phone }) {
  ...
  try {
    ...
    if (!res.ok || !data?.ok) {
      return { ok: false, error: data?.error || `http_${res.status}`, gateway: data?.gateway };
    }
    return { ok: true, gateway: data.gateway };
  } catch (e) {
    return { ok: false, error: e?.name === 'TimeoutError' ? 'timeout' : 'network_error' };
  }
}
